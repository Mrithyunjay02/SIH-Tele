...
const Sidebar = ({ handleLogout }) => {
    ...
    return (
        <div className="w-64 bg-gray-800 flex flex-col shrink-0">
            <div className="p-6 text-2xl font-bold border-b border-gray-700">Sehat Saathi</div>
            <nav className="flex-1 p-4 space-y-2">
                {navItems.map(item => (
                    <Link key={item.path} to={item.path} className={`flex items-center p-3 rounded-lg transition-colors ${location.pathname === item.path ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'}`}>
                        {item.icon}
                        {item.name}
                    </Link>
                ))}
